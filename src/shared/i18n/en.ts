export const en = {
	'welcome.title': 'Welcome to Shep',
	'welcome.body': 'Add a service with the + in the bar on the left.',
	'rail.add': 'Add a service',
	'rail.workspaces': 'Workspaces',
	'rail.allServices': 'All Services',
	'rail.unreadElsewhere': 'Something is unread in another workspace',
	'rail.dontDisturb': "Don't Disturb",
	'rail.dontDisturb.on': "Don't Disturb is on",
	'titlebar.back': 'Back',
	'titlebar.forward': 'Forward',
	'titlebar.reload': 'Reload',
	'titlebar.home': 'Home',
	'titlebar.actualSize': 'Actual Size',
	'titlebar.find': 'Find in page',
	'find.placeholder': 'Find in page',
	'find.previous': 'Previous match',
	'find.next': 'Next match',
	'find.none': 'No matches',
	'dialog.add.title': 'Add a Service',
	'dialog.edit.title': 'Edit Service',
	'dialog.address': 'Address',
	'dialog.address.placeholder': 'web.whatsapp.com',
	'dialog.address.search': 'Type an address, or look for a service below',
	'dialog.catalogue': 'Services to start from',
	'dialog.catalogue.messaging': 'Messaging',
	'dialog.catalogue.work': 'Work and meetings',
	'dialog.catalogue.mail': 'Mail and calendar',
	'dialog.catalogue.tasks': 'Tasks and documents',
	'dialog.catalogue.assistants': 'Assistants',
	'dialog.catalogue.support': 'Customer support',
	'dialog.catalogue.music': 'Music',
	'dialog.address.invalid': 'Type the address of the service, such as web.whatsapp.com',
	'dialog.name': 'Name',
	'dialog.name.placeholder': 'Taken from the address when left empty',
	'dialog.workspace': 'Workspace',
	'dialog.workspace.every': 'All workspaces',
	'dialog.newWorkspace.title': 'New Workspace',
	'dialog.renameWorkspace.title': 'Rename Workspace',
	'dialog.workspaceName.placeholder': 'Work, Personal…',
	'dialog.workspaceIcon.title': 'Workspace Icon',
	'dialog.workspaceIcon.initials': 'Initials',
	'dialog.cancel': 'Cancel',
	'dialog.add': 'Add',
	'dialog.save': 'Save',
	'dialog.close': 'Close',
	'rail.preferences': 'Preferences',
	'prefs.title': 'Preferences',
	'prefs.appearance': 'Appearance',
	'prefs.window': 'Window',
	'prefs.services': 'Services',
	'prefs.security': 'Security',
	'prefs.advanced': 'Advanced',
	'prefs.theme': 'Style',
	'prefs.theme.system': 'Follow the system',
	'prefs.theme.light': 'Light',
	'prefs.theme.dark': 'Dark',
	'prefs.language': 'Language',
	'prefs.language.auto': "The system's",
	'prefs.startMinimized': 'Start minimized',
	'prefs.closeBehaviour': 'Closing the window',
	'prefs.closeBehaviour.tray': 'Keeps Shep running in the background',
	'prefs.closeBehaviour.quit': 'Quits Shep',
	'prefs.trayIcon': 'Show an icon in the system tray',
	'prefs.alwaysOnTop': 'Keep the window above others',
	'prefs.startWithSystem': 'Start when you log in',
	'prefs.openOnStart': 'Open on start',
	'prefs.openOnStart.welcome': 'The welcome page',
	'prefs.openOnStart.last': 'The last service used',
	'prefs.spellcheck': 'Check spelling',
	'prefs.spellcheckLanguages': 'Spelling languages',
	'prefs.spellcheckLanguages.automatic': 'Automatic: {languages}',
	'prefs.spellcheckLanguages.picked': 'Remove them all to go back to the automatic ones.',
	'prefs.spellcheckLanguages.add': 'Add a language…',
	'prefs.spellcheckLanguages.remove': 'Remove {language}',
	'prefs.lockPassword': 'Lock password',
	'prefs.lockPassword.hint': 'Locks Shep with Alt+Shift+L, or when it starts.',
	'prefs.lockPassword.set': 'Set Password…',
	'prefs.lockPassword.change': 'Change Password…',
	'prefs.lockOnStart': 'Ask for it when Shep starts',
	'prefs.proxy': 'Use a proxy',
	'prefs.proxyHost': 'Host',
	'prefs.proxyPort': 'Port',
	'prefs.userAgent': 'User agent',
	'prefs.userAgent.hint': "Empty: Chromium's own. A custom one is refused by some logins, such as Cloudflare's.",
	'prefs.hardwareAcceleration': 'Hardware acceleration',
	'prefs.relaunch': 'Takes effect when Shep restarts.',
	'prefs.relaunchNow': 'Restart Now',
	'prefs.about': 'About Shep',
	'prefs.checkForUpdates': 'Check for Updates',
	'prefs.reportIssue': 'Report an Issue',
	'prefs.unreadReport': 'Unread Report',
	'prefs.clearCache': 'Clear Cache',
	'prefs.removeAllServices': 'Remove All Services…',
	'about.version': 'Version',
	'about.body': 'One window for the web apps you already use in the browser.',
	'about.licence': 'GNU General Public License v3',
	'report.title': 'Unread Report',
	'report.body': 'What each service\'s title says, and the count taken from it.',
	'report.service': 'Service',
	'report.pageTitle': 'Title',
	'report.count': 'Count',
	'report.empty': 'No service is open.',
	'password.title': 'Lock Password',
	'password.new': 'New password',
	'password.repeat': 'Repeat it',
	'password.mismatch': 'The two do not match',
	'password.remove': 'Remove Password',
	'lock.title': 'Shep is locked',
	'lock.password': 'Password',
	'lock.unlock': 'Unlock',
	'lock.wrong': 'That is not the password',
	'rail.lock': 'Lock',
	'share.title': 'Share Your Screen',
	'share.body': 'Choose what the service will see.',
	'share.cancel': 'Cancel',
	'share.share': 'Share',
	'menu.reload': 'Reload',
	'menu.notifications': 'Notifications',
	'menu.sound': 'Sound',
	'menu.enabled': 'Enabled',
	'menu.edit': 'Edit…',
	'menu.moveToWorkspace': 'Move to Workspace',
	'menu.everyWorkspace': 'All Workspaces',
	'menu.remove': 'Remove…',
	'menu.developerTools': 'Developer Tools',
	'menu.allServices': 'All Services',
	'menu.newWorkspace': 'New Workspace…',
	'menu.renameWorkspace': 'Rename…',
	'menu.changeIcon': 'Change Icon…',
	'menu.deleteWorkspace': 'Delete',
	'menu.copyLink': 'Copy Link',
	'menu.copyEmail': 'Copy Email Address',
	'menu.openInBrowser': 'Open Link in Browser',
	'menu.copyImage': 'Copy Image',
	'menu.copyImageUrl': 'Copy Image URL',
	'menu.search': 'Search with Google',
	'menu.noSuggestions': 'No Spelling Suggestions',
	'menu.addToDictionary': 'Add to Dictionary',
	'menu.cut': 'Cut',
	'menu.copy': 'Copy',
	'menu.paste': 'Paste',
	'tray.show': 'Show Shep',
	'tray.hide': 'Hide Shep',
	'tray.dontDisturb': "Don't Disturb",
	'tray.quit': 'Quit',
	'permission.title': 'Permission request',
	'permission.camera': '{name} wants to use your camera and microphone.',
	'permission.screen': '{name} wants to capture your screen.',
	'permission.detail': 'Shep remembers this answer for this service.',
	'permission.allow': 'Allow',
	'permission.block': 'Block',
	'remove.message': 'Remove {name}?',
	'remove.detail': 'Its sign-in and everything it stored on this computer go with it.',
	'remove.confirm': 'Remove',
	'removeAll.message': 'Remove every service?',
	'removeAll.detail': 'Their sign-ins and everything they stored on this computer go with them.',
	'removeAll.confirm': 'Remove All',
	'deleteWorkspace.message': 'Delete {name}?',
	'deleteWorkspace.detail': 'Its services stay, and show in every workspace.',
	'deleteWorkspace.confirm': 'Delete',
	'updates.upToDate': 'Shep is up to date.',
	'updates.latest': 'Version {version} is the latest.',
	'updates.ready': 'Shep {version} is ready.',
	'updates.restart': 'Restart Shep to use it.',
	'updates.restartNow': 'Restart Now',
	'updates.later': 'Later',
	'updates.failed': 'Shep could not check for updates.',
	'updates.packagedOnly': 'Updates come to packaged builds only.'
} as const;
