...
**The home tab's service list is a dataview, not a grid**, and that choice is worth keeping. A grid draws each row as a `<table>` Ext sizes in pixels, so margins and rounded corners fight the layout rather than compose with it -- three rounds of CSS failed to make rows look like cards before the component was swapped. A dataview renders a template we own, so the row's markup is whatever the design needs and Ext still binds the store and dispatches clicks. Where the grid spent two action columns and a check column, `onServiceListClick` reads the `data-act` of whatever was hit.

Two things went with the grid. Rows no longer group by `align`, because the rail already shows that split above and below its fill. And renaming happens in the Edit window rather than in place, so `onRenameService` is gone. The same reasoning applies to the catalogue, which was already a dataview.

**The home tab** opens on how much is waiting, not on a shop. It is the unread summary plus the enabled-services grid at full width; the catalogue that used to take two thirds of it is a hidden floating panel with `itemId: 'catalogue'`, opened by the `+` in the rail through `MainController.openCatalogue`. Floating keeps it out of the hbox while leaving it a child of the view, which is the whole reason it works: its filters, its search and its item click are string handlers that resolve against `MainController`, and a window of its own would have needed a second controller.

The summary is written by `updateUnreadSummary` in `app/Application.js`, called from `updateTotalNotifications`. It is called rather than being its own config hook because `totalNotifications` already had one, and a second entry of that name in the same object literal is simply overwritten -- which is exactly what happened on the first attempt, silently. `Redil.util.UnreadCounter` grew `getServicesWithUnread` for the second line of it.

**Preferences** are five sections -- Appearance, Window, Services, Security, Advanced -- in a left tab panel, not the single scroll of fourteen controls they used to be. Every field still lives inside one form, so Save reads them all in a single `getFieldValues`; `deferredRender: false` is what makes that true, by rendering every section whether or not it is on screen. Take it off and the proxy and master password fields silently stop saving. The nav's width is a `tabBar: { width: 176 }` config rather than a rule in the stylesheet, because Ext places the body against the width it laid the bar out with: a CSS-only width shrank what was painted and left a white gutter beside it.
